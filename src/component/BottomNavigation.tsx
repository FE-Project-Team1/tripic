...
function BottomNavigation() {
  return (
    <ul className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center py-2 px-[6px] z-50 h-15">
      <List svg={iconHomeFill} text={'홈'} on={true} />
      <List svg={iconMessageCircle} text={'채팅'} on={false} />
      <List svg={iconEdit} text={'게시물 작성'} on={false} />
      <List svg={iconUser} text={'프로필'} on={false} />
    </ul>
  );
}
